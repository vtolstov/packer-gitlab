...
lxc-info -n gitlab || lxc-clone -s -B aufs trusty-base gitlab

for d in juju mysql gitlab; do
  lxc-start -d -n $d;
done
...
GL_IP=$(lxc-info -n gitlab -i | awk '{print $2}')
...
juju add-machine ssh:ubuntu@${GL_IP} #2
...
juju deploy --repository=charms/ local:trusty/gitlab --to 2 || juju deploy --repository=charms/ local:trusty/gitlab --to 2 || exit 1;
juju set gitlab admin_password=5iveL!fe
juju add-relation mysql gitlab
...
for s in mysql gitlab haproxy; do
    while true; do
